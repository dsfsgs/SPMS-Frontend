      ...
      <q-card flat bordered>
        ...
        <q-card-section class="q-pa-sm">
          ...
          <div class="row q-col-gutter-sm">
            ...
            <div class="col-12 col-md-8" v-if="currentEmployee.employeeId">
              <div class="row q-col-gutter-sm">
                  ...
                  <q-card-section class="q-pa-sm bg-grey-2">
                    <div class="row items-center justify-between">
                      <div class="text-subtitle2">
                        Performance Standard {{ getStandardOutputLabel(standard) }} {{ index + 1 }}:
                        ...
                        <div class="row q-col-gutter-md">
                          ...
                          <div class="col-md-6">
                            <q-card flat bordered class="full-height">
                              <q-card-section class="q-pa-sm">
                                <div class="text-subtitle2">Main Function Output (MFO) Details</div>
                              </q-card-section>
                              <q-separator />
                              <q-card-section class="q-pa-sm">
                                <div class="column q-gutter-sm">
...
export default {
  ...
  setup() {
    ...
    // ===========================================================================
    // 8.5 COMPETENCY MODAL COMPUTED PROPERTIES (ADDED FOR SELECT ALL)
    // ===========================================================================
    ...
    // ===========================================================================
    // 9. QUANTITY CONFIG FUNCTIONS - Updated with Support category exception
    // ===========================================================================
    ...
    // ===========================================================================
    // 10. METHODS THAT USE STORES (these will be called after initialization)
    // ===========================================================================
    ...
    const fetchHeadMfos = async () => {
      ...
      if (!empData) {
        ...
        const headEmp = allEmployees.find(
          (emp) => (emp.job_title || emp.jobTitle || '').toLowerCase().trim() === 'department head',
        )
      ...
      const payload = {
        employee: {
          ...
          job_title: empData.job_title || empData.jobTitle || 'Department Head',
    ...
    // ===========================================================================
    // UPDATED: generateSuccessIndicator - now uses selected output instead of outputName
    // ===========================================================================
    const generateSuccessIndicator = (index) => {
      ...
      indices.forEach((i) => {
        ...
        // Helper to get output name from selected output ID
        const getOutputName = () => {
        ...
        // Build success indicator with selected output
    ...
    // ===========================================================================
    // 9.5 COMPETENCY MODAL METHODS (ADDED FOR SELECT ALL)
    // ===========================================================================
    ...
    const filterCompetencies = (val, update, rowIndex) => {
      ...
      update(() => {
        const needle = (val || '').toLowerCase()
        const allOptions = competencyOptions.value
        ...
        // Get already selected competencies of this type
        const standard =
          currentEmployee.value.performanceStandards[currentStandardIndexForCompetency.value]
        const currentType = competencyType.value
        const selectedInType = standard?.competencies[currentType]?.map((c) => c.code) || []

        // Get competencies selected in other modal rows
        const selectedInModal = competencySelections.value
          .map((sel, i) =>
        ...
        const taken = [...selectedInType, ...selectedInModal]

        filteredCompetencyOptionsByRow.value[rowIndex] = filtered.filter(
          (c) => !taken.includes(c.code),
        )
      })
    }

    const getAvailableCompetenciesForType = (type, standardIndex) => {
      const standard = currentEmployee.value.performanceStandards[standardIndex]
      if (!standard) return []

      // Get all selected competencies for this type
      const selectedCodes = standard.competencies[type]?.map((c) => c.code) || []

      // Get competencies available from the store
      const allAvailable = competencyOptions.value // Assuming this is already filtered by SG

      // Filter out selected ones
      return allAvailable.filter((c) => !selectedCodes.includes(c.code))
    }
    ...
    const addAllSelectedCompetencies = () => {
      ...
      competencySelections.value.forEach(({ selectedCompetency: sc, selectedLevel: sl }) => {
        ...
        // Check if competency already exists in the current type
    ...
    // ===========================================================================
    // UPDATED: onQuantityOptionSelect - refreshes restriction when quantity type changes
    // ===========================================================================
    const onQuantityOptionSelect = (value, index) => {
      ...
      if (value === 'B' || value === 'C') {
        ...
        if (
          ...
          setTimeout(() => checkAndShowCascadeModal(index), 100)
        }
      }
      ...
      if (
        ...
        setTimeout(() => checkAndShowCascadeModal(index), 200)
      }
    }
    ...
    const computeQuantities = (type = null, index = null) => {
      ...
      if (currentType === 'B') {
        ...
        if (
          ...
          setTimeout(() => checkAndShowCascadeModal(idx), 300)
        }
    ...
    // Watch for changes to output selection to regenerate success indicator
    watch(
      ...
      async (changes) => {
        ...
        for (const change of changes) {
          if (change.mfo && change.output && change.indicator) {
            ...
            await checkAndShowCascadeModal(change.index)
          }
        }
      },
      { deep: true },
    ...
    return {
      ...
      shouldShowOutput,
      getAvailableCompetenciesForType,
