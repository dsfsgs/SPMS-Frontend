      ...
      <template #body-cell-action="props">
        <q-td :props="props" class="text-center">
          <div class="row justify-center q-gutter-xs">
            ...
            <!-- Status Update Button - Only show for applicable statuses -->
            <q-btn
              v-if="shouldShowUpdateButton(props.row.status)"
              flat
              round
              color="primary"
              icon="sync_alt"
...
import OPCRModal from 'src/components/OPCRModalPMT.vue'
...
// =========================
const isReceivedTarget = (status) => {
  if (!status) return false
  const s = status.toLowerCase().trim()
  return s === 'received target' || s.includes('received target')
}

const isPrevalidatedAccomplishment = (status) => {
  if (!status) return false
  const s = status.toLowerCase().trim()
  return s === 'prevalidated accomplishment' || s.includes('prevalidated accomplishment')
}
...
const getStatusOptions = (status) => {
  ...
  // Check for Received Target
  if (isReceivedTarget(status)) {
    return [
      {
  ...
  // Check for Prevalidated Accomplishment
  if (isPrevalidatedAccomplishment(status)) {
    return [
      {
...
const shouldShowUpdateButton = (status) => {
  if (!status) return false
  // Only show for Received Target and Prevalidated Accomplishment
  return isReceivedTarget(status) || isPrevalidatedAccomplishment(status)
}
...
const openUpdateModal = (row) => {
  ...
  newStatus.value = options.length > 0 ? options[0].value : null
...
/* Neumorphic button styling - same as SPMSDashboard */
.neu-button {
  border-radius: 50%;
  background: #f7fafc;
  box-shadow:
