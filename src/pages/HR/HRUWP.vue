  ...
  <q-page padding>
    ...
    >
      ...
      <template #body-cell-action="props">
        <q-td :props="props" class="text-center">
          <!-- Only show action button if status is "Received Target" -->
          <template v-if="props.row.unitworkplan_status === 'Received Target'">
            <q-btn
              flat
              round
              color="primary"
...
// Only "Received Target" status can be updated to "Reviewed Target" or "Returned Target"
  ...
  'received target': [
    {
      ...
      color: 'green-7',
      description: 'Mark the UWP as reviewed and approved.',
    },
    {
...
const filteredRows = computed(() => {
  ...
  if (!q) return uwpStore.hrRecords
  return uwpStore.hrRecords.filter((r) =>
...
const handleUpdateStatus = async () => {
  ...
  try {
    ...
    if (selectedYear.value && selectedSemester.value) {
      await uwpStore.fetchHRRecords(selectedYear.value, selectedSemester.value)
    }
  } catch (error) {
    $q.notify({
      type: 'negative',
...
watch([selectedYear, selectedSemester], ([y, s]) => {
  if (y && s) uwpStore.fetchHRRecords(y, s)
})
