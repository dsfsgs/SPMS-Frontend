              ...
              <div class="col">
                ...
                <div class="text-subtitle2 text-weight-medium">
                  ...
                  <q-badge
                    ...
                    :label="formatStatusDisplay(currentStatus)"
                    class="q-ml-xs"
                    style="font-size: 0.9rem; padding: 4px 12px; border-radius: 12px"
                  />
                </div>
...
export default {
  ...
  setup(props, { emit }) {
    ...
    const getStatusBadgeColor = (status) => {
      ...
      if (s === 'received target' || s === 'received accomplishment') return 'indigo-6'
      if (s === 'returned target' || s === 'returned accomplishment') return 'red-6'
      if (s === 'reviewed target' || s === 'reviewed accomplishment') return 'purple-6'
      if (s === 'prevalidated target') return 'cyan-7'
      if (
        s === 'calibrated/validated target' ||
        s === 'calibrated/validated accomplishment' ||
        s === 'validated/calibrated accomplishment'
      )
        return 'green-7'
    ...
    const availableStatusOptions = computed(() => {
      return getStatusOptions(currentStatus.value)
    })
    ...
    const openStatusModal = () => {
      ...
      const options = getStatusOptions(currentStatus.value)
      selectedStatus.value = options.length > 0 ? options[0].value : null
    ...
    const updateStatus = async () => {
      ...
      // Get the unitworkplan record from the store
      const unitworkplan = store.data?.office?.unitworkplan

      if (!unitworkplan || !unitworkplan.id) {
        $q.notify({
      ...
      try {
        // Call monitorStore to update status
        const response = await monitorStore.updateUWPStatus(
        ...
        if (response) {
          ...
          // Refresh the current division data
          ...
          $q.notify({
            message: `Status updated to "${selectedStatus.value}" successfully.`,
            color: 'positive',
            position: 'top',
            icon: 'check_circle',
            timeout: 3000,
        ...
        monitorStore.error = errorMessage
      } finally {
        statusLoading.value = false
      }
    }
